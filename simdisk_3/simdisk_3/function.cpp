...
void Directory::delete_file(const char* filename)
{
	...
	//判断i节点是否正在被写入
	if (sharedm_read(df_inode)) {
		cout << "该目录/文件正在被其他进程写入，无法进行读写" << endl;
		return;
	}

	//判断用户权限
	if (current_usertype == USER && inode_table[df_inode].uid != current_user || inode_table[df_inode].uid == 0) {
		cout << "没有权限删除文件" << filename << endl;
		return;
	}
	//写入i节点使用表
	int tempindex = sharedm_alloc(df_inode);
	if (tempindex == -1) {
		cout << "共享区正在被其他进程写入，无法进行读写" << endl;
		return;
	}
	//释放块和i节点
...
void Directory::create_file(const char* filename, char *content, int filelen, fileAccess fileaccess)
{
	...
	if (cf_addr < 0) {
		cout << "磁盘空间不足，创建文件失败" << endl;
		return;
	}
	cf_inode = inode_alloc();
	if (cf_inode < 0) {
		cout << "i节点分配失败，创建文件失败" << endl;
		return;
	}
...
bool login()
{
	...
	cout << "请输入账号和密码" << endl;
	int login_id;
	char login_password[MAX_PASSWORD_SIZE];
	cout << "账号: ";
	cin >> login_id;
	if (cin.fail()) {
		cout << "账号应为数字" << endl << endl;
		cin.clear();
		cin.ignore(1024, '\n');
		return false;
	}
	...
	//检测是否该用户已经登录
	if (get_logined_user(login_id)) {
		cout << "该账号已在其他进程登录" << endl << endl;
		return false;
	}

	//在用户表中查看账号密码是否正确
	for (i = 0; i < MAX_USER_NUM; i++) {
		if (login_id == user_group[i].getUserid() && strcmp(login_password, user_group[i].getPassword()) == 0) {
			current_user = login_id;
			current_usertype = user_group[i].getUsertype();
			if (!set_logined_user(current_user, true)) {
				cout << "设置用户登录信息失败" << endl;
				return false;
			}
